...
class FixedMCT(scheduler.DynamicScheduler):
  """
  ...
  def schedule(self, simulation, changed):
    ...
    for task in simulation.tasks:
      ...
      if target_host:
        self._task_count[target_host] += 1
    for task in simulation.tasks[csimdag.TaskState.TASK_STATE_DONE, csimdag.TaskState.TASK_STATE_RUNNING]:
      target_host = self._target_hosts.get(task)
      if target_host:
        self._task_count[target_host] -= 1
    """
