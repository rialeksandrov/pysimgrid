...
class FixedBatchScheduler(scheduler.DynamicScheduler):
  """
  ...
  * Sufferage prioritizes tasks with maximum difference between ECT on 2 best hosts
  ...
  def schedule(self, simulation, changed):
    ...
    for task in unscheduled:
      self._target_hosts[task] = None
    for _ in range(len(unscheduled)):
      possible_schedules = []
      for task in unscheduled:
        if self._target_hosts[task]:
          continue
  ...
  def get_ect(clock, task, host):
    ...
    #print(clock, task.name, host.name, parent_connections)
...
class FixedBatchMin(FixedBatchScheduler):
  """
...
class FixedBatchMax(FixedBatchScheduler):
  """
...
class FixedBatchSufferage(FixedBatchScheduler):
  """
